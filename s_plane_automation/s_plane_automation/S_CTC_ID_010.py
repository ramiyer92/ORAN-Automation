...
def session_login(host, port, user, password):
    try:
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password) as m:
            ...
            pdf.add_page() 
            STARTUP.STORE_DATA('Retrieving the Locked State of O-RU',Format = True,PDF = pdf)
            SYNC = '''<filter xmlns="urn:ietf:params:xml:ns:netconf:base:1.0">
            <sync xmlns="urn:o-ran:sync:1.0">
            </sync>
            </filter>
            ...
            pdf.add_page()
            STARTUP.STORE_DATA("Disconnect the Ptp and SyncE & Capcturing Notification",Format = True,PDF = pdf)
            ...
            pdf.add_page()
            STARTUP.STORE_DATA('Retrieving the Holdover State of O-RU',Format = True,PDF = pdf)
...
def Result_Decleration(host, port, user, password):
    try:
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password) as m:
            ...
            STARTUP.STORE_DATA(f"{'Ptp_lock_state STATUS' : <50}{'=' : ^20}{'PASS' : ^20}" if Ptp_lock_state == "UNLOCKED" or "UNLOCKED" else f"{'Ptp_lock_state STATUS' : <50}{'=' : ^20}{'FAIL' : ^20}",Format = False,PDF = pdf)
            
            Ptp_state=dict_Sync['data']['sync']['ptp-status']['sources']['state']
            STARTUP.STORE_DATA(f"{'Ptp_state STATUS' : <50}{'=' : ^20}{'PASS' : ^20}" if Ptp_state == "DISABLED" else f"{'Ptp_state STATUS' : <50}{'=' : ^20}{'FAIL' : ^20}",Format = True,PDF = pdf)
...
if __name__ == '__main__':
    ...
    try:
        ...
        for i in lines:
            STARTUP.STORE_DATA(i,end="",format = False,PDF = pdf)
        ...
        scp.close()
        CREATE_LOGS(pdf,name = "S_CTC_ID_010")
