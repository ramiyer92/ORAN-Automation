    ...
    try:
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password) as m:
            ...
            if dict_data['nc:rpc-reply']['nc:ok']== None:
                STARTUP.STORE_DATA('Ok',Format = True,PDF = pdf)
            ...
            if element == "detected":
                ...
                while True:
                    ...
                    if NF1=='sync-state':
                        ...
                        break        
                pdf.add_page()
                STARTUP.STORE_DATA('\t\t\t Retriving the nominal Clock class and Degraded within range ssm code',Format = True,PDF = pdf)  
                SYNC = '''<filter xmlns="urn:ietf:params:xml:ns:netconf:base:1.0">
                <sync xmlns="urn:o-ran:sync:1.0">
                </sync>
                </filter>
                ...
                #give clock class 7 from paragon
                STARTUP.STORE_DATA("\t\t\t configure clock class 7 from Master",Format = True,PDF = pdf)
                ...
                #give clock class 135 from paragon
                STARTUP.STORE_DATA("\t\t\t configure clock class 135 from Master",Format = True,PDF = pdf)
                ...
                STARTUP.STORE_DATA("Retriving the ptp and synce and sync state",Format = True,PDF = pdf)
                ...
                STARTUP.STORE_DATA("Device Not Detected",Format = True,PDF = pdf)
...
if __name__ == '__main__':
    ...
    try:
        ...
        ssh_client.connect(hostname=li[0],username='root',password='vvdn5gbudeveloperaccess')

        stdin, stdout, stderr = ssh_client.exec_command(command)
        lines = stdout.readlines()
        for i in lines:
            STARTUP.STORE_DATA(i,end="",Format = True,PDF = pdf)
        ...
        scp.close()
        CREATE_LOGS(pdf,name = "S_CTC_ID_003")
