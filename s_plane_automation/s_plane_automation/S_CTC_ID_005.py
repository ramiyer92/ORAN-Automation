    ...
    try:
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password) as m:
            ...
            if element == "detected":
                ...
                STARTUP.STORE_DATA("Change SSM_CODE to DNU from master",Format = True,PDF = pdf)
                ...
                STARTUP.STORE_DATA("Set SSM-CODE to DNU from Master")
...
def Result_Decleration(host, port, user, password):
    try:
        with manager.connect(host=host, port=port, username=user, hostkey_verify=False, password=password) as m:
            ...
            STARTUP.STORE_DATA(f"{'SyncE_lock_state STATUS' : <50}{'=' : ^20}{'PASS' : ^20}" if SyncE_lock_state == "UNLOCKED" else f"{'SyncE_lock_state STATUS' : <50}{'=' : ^20}{'FAIL' : ^20}",Format = True,PDF = pdf)

            SyncE_state=dict_Sync['data']['sync']['synce-status']['sources']['state']
            STARTUP.STORE_DATA(f"{'SyncE_State STATUS' : <50}{'=' : ^20}{'PASS' : ^20}" if SyncE_state == "NOK" else f"{'SyncE_state STATUS' : <50}{'=' : ^20}{'FAIL' : ^20}",Format = True,PDF = pdf)
    except Exception as e:
        STARTUP.STORE_DATA(e,)
        exc_type, exc_obj, exc_tb = sys.exc_info()
        STARTUP.STORE_DATA('Error Line Number:',{exc_tb.tb_lineno},Format = True,PDF = pdf)
...
if __name__ == '__main__':
    ...
    try:
        ...
        scp.close()
        CREATE_LOGS(pdf,name = "S_CTC_ID_005")
